<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="教学设计ID" prop="designId">
        <el-input
          v-model="queryParams.designId"
          placeholder="请输入教学设计ID"
          clearable
    ...
    <el-table v-loading="loading" :data="contentList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="教学设计ID" align="center" prop="designId" />
      <el-table-column label="内容类型(overview:课程概述,chapters:课程章节,assessment:考核方案,scoring:评分标准,outcome:预计成果)" align="center" prop="contentType" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="教学设计ID" prop="designId">
          <el-input v-model="form.designId" placeholder="请输入教学设计ID" />
        </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        designId: [
          { required: true, message: "教学设计ID不能为空", trigger: "blur" }
        ],
        contentType: [
          { required: true, message: "内容类型(overview:课程概述,chapters:课程章节,assessment:考核方案,scoring:评分标准,outcome:预计成果)不能为空", trigger: "change" }
        ],
        content: [
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除教学设计内容编号为"' + ids + '"的数据项？').then(function() {
