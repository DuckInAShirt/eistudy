<template>
  <div class="job-info-chat">
    <div class="page-hero" :style="bgStyle('job-info')">
      <h2>招聘信息</h2>
      <p>岗位趋势、行业方向与投递建议智能问答</p>
    </div>
...
// 招聘信息的 Dify API Key 和 URL
const JOB_INFO_API_KEY = DIFY_API_KEYS['job-info'] || DIFY_API_KEYS.default
const JOB_INFO_API_URL = DIFY_API_URLS['job-info'] || DIFY_API_URLS.default
...
export default {
  ...
  created() {
    this.messages.push({
      id: Date.now(),
      role: 'assistant',
      content: '你好！我是**招聘信息**AI助手，可以为你解答岗位趋势、行业方向、投递建议等问题。有什么可以帮助你的吗？'
    })
  },
  methods: {
    bgStyle(type) {
      const map = {
        'job-info': '/img/easy-square/job-info.svg'
      }
      return {
    ...
    async sendMessage() {
      ...
      await difyChatStream(
        ...
        JOB_INFO_API_KEY,  // API Key
        JOB_INFO_API_URL,  // Base URL
